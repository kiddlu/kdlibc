...
int md5(const unsigned char *ibuf, uint32_t isize, unsigned char *obuf)
{
    MD5_CTX ctx;

    md5_init(&ctx);
    md5_update(&ctx, ibuf, isize);
    md5_final(&ctx, obuf);
...
int main(int argc, char* argv[])
{
    ...
    unsigned char md_buf[MD5_BLOCK_SIZE];
    ...
    print_hex(md_buf, sizeof(md_buf));
    printf("  %s\n", argv[1]);
    return 0;
}
